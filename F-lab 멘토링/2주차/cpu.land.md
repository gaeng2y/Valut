...
OS와 상호 작용하는 코드를 보거나 작성한 적있다면 `open`, `read`, `fork` 그리고 `exit`와 같은 기능을 인식할 것이다. 몇 가지 추상화 계층 아래에서 이러한 기능은 모두 System call을 사용하여 OS에 도움을 요청한다. Syscall은 **프로그램이 사용자 공간에서 커널 공간으로 전환을 시작하고 프로그램 코드에서 OS 코드로 점프할 수 있도록 하는 특별한 절차**이다.
...
### Wrapper API: 인터럽트 추상화
Syscall에 대해 지금까지 우리가 알고 있는 내용은 다음과 같다.
- 유저 모드 프로그램은 I/O나 메모리에 직접 접근할 수 없다. 그들은 OS에 외부 세계와 상호 작용하는 데 도움을 요청해야 한다.
- 프로그램은 INT 및 IRET와 같은 특수 기계 코드 명령어를 사용하여 OS에 제어를 위임할 수 있다.
- 프로그램은 권한 수준을 직접 전환할 수 없다. 소프트웨어 인터럽트는 프로세서가 OS 코드에서 점프할 위치와 함께 OS에 의해 사전 구성되었기 때문에 안전하다. IVT는 커널 모드에서만 구성할 수 있다.

프로그램은 Syscall을 트리거할 때 OS에 데이터를 전달해야 한다. OS는 Syscall 자체에 필요한 데이터와 함께 실행할 특정 시스템 호출(예: 열리는 파일 이름)을 알아야 한다. 이 데이터를 전달하는 메커니즘은 OS와 아키텍처에 따라 다르지만 일반적으로 인터럽트를 트리거하기 전에 특정 레지스터나 스택에 데이터를 배치하여 수행된다.

Syscall의 기기별 차이와 프로그램 작성의 복잡성 때문에, 프로그래머가 직접 시스템 호출을 구현하는 것은 비현실적이며 운영 체제 변경 시 호환성 문제도 발생한다.

![image6](https://cpu.land/images/syscall-architecture-differences.png)

그래서 영 체제는 이러한 인터럽트 위에 추상화 계층을 제공합니다. Unix 계열 시스템에서는 libc, Windows에서는 ntdll.dll이라는 라이브러리가 필요한 어셈블리 명령을 감싸는 재사용 가능한 고수준 라이브러리 함수들을 제공합니다. 이러한 라이브러리 함수 호출 자체는 커널 모드로 전환되지 않으며, 표준 함수 호출일 뿐입니다. 라이브러리 내부에서는 어셈블리 코드가 실제로 커널로 제어를 넘기며, 이는 감싸는 라이브러리 서브루틴보다 훨씬 플랫폼 의존적입니다.

C 언어에서 Unix 계열 시스템에서 `exit(1)`을 호출할 때, 그 함수는 시스템 호출의 오피코드와 인수를 올바른 레지스터나 스택 등에 배치한 후 인터럽트를 트리거하는 기계어를 내부적으로 실행하고 있습니다. 컴퓨터는 정말 멋지네요!
### 속도의 필요성 / CISC-y를 얻자
1. 많은 CISC 아키텍처, 특히 x86-64는 시스템 호출을 위한 최적화된 명령어(SYSCALL, SYSENTER)를 포함하고 있으며, 이는 INT 0x80보다 효율적입니다.
    
2. AMD와 Intel의 프로세서는 이러한 명령어의 호환성이 다소 다르며, 64비트 프로그램에는 SYSCALL이, 32비트 프로그램에는 SYSENTER가 더 적합합니다.
    
3. RISC 아키텍처, 특히 AArch64는 시스템 호출과 소프트웨어 인터럽트를 위해 단 하나의 인터럽트 명령어를 사용하며, 이는 Apple Silicon 기반 Mac에서 사용됩니다.

## Recap
- 프로세서는 무한 **Fetch-Execute Cycle**에서 명령을 실행하며 운영 체제나 프로그램에 대한 개념이 없습니다. 일반적으로 레지스터에 저장되는 프로세서 모드에 따라 실행될 수 있는 명령이 결정됩니다. 운영 체제 코드는 커널 모드에서 실행되고 사용자 모드로 전환되어 프로그램을 실행합니다.
- 바이너리를 실행하기 위해 운영 체제는 사용자 모드로 전환하고 프로세서가 RAM의 코드 진입점을 가리키도록 합니다. 사용자 모드의 권한만 있기 때문에 세상과 상호 작용하려는 프로그램은 OS 코드로 이동하여 도움을 받아야 합니다. 시스템 호출은 프로그램이 사용자 모드에서 커널 모드로, 그리고 OS 코드로 전환하는 표준화된 방법입니다.
- 프로그램은 일반적으로 공유 라이브러리 함수를 호출하여 이러한 syscall을 사용합니다. 이는 제어를 OS 커널 및 스위치 링으로 전송하는 소프트웨어 인터럽트 또는 아키텍처별 syscall 명령어에 대한 기계어 코드를 래핑합니다. 커널은 해당 작업을 수행하고 사용자 모드로 다시 전환한 다음 프로그램 코드로 돌아갑니다.
# Slice Dat Time
OS를 구축 중이고 유저가 동시에 여러 프로그램을 실행할 수 있기를 원한다고 가정해보자. 하지만 멀티 코어 프로세서가 없기 때문에 CPU는 한 번에 하나의 명령만 실행할 수 있다.

프로세스가 CPU를 차례로 사용하도록하여 병렬성을 가짜로 만들 수 있다는 것을 알아냈다. 프로세스를 순환하고 각 프로세스에서 몇 가지 명령을 실행하면 CPU를 잡아먹는 단일 프로세스 없이 모두 응답할 수 있다.

하지만 프로그램 코드에서 프로세스 전환으로 제어권을 다시 가져오려면 어떻게 해야할까? 컴퓨터에 타이머 칩이 있다는 것을 알게됐는데, 일정 시간이 지나면 OS 인터럽트 핸들러로의 전환을 트리거하도록 타이머 칩을 프로그래밍할 수 있다.
## 하드웨어 인터럽트
앞에서 우리는 소프트웨어 인터럽트를 이용하여 유저 프로그램에서 OS로 직접 제어하는 방법에 대해서 알아봤다. 이는 프로그램에 의해 자발적으로 트리거되기 때문에 "소프트웨어" 인터럽트라고 한다. 일반적으로 fetch-execute 사이클에서 프로세서에 의해 실행되는 기계 코드는 제어를 커널로 전환하라고 지시한다.

![image7](https://cpu.land/images/keyboard-hardware-interrupt.png)

OS 스케줄러는 PIT와 같은 타이머 칩을 사용하여 멀티태스킹을 위한 하드웨어 인터럽트를 트리거한다.

1. 프로그램 코드로 점프하기 전에 OS는 일정 시간 후에 인터럽트를 트리거하도록 타이머 칩을 설정한다.
2. OS는 유저 모드로 전환되고 프로그램의 다음 명령어로 점프한다.
3. 타이머가 경과되면 하드웨어 인터럽트를 트리거하여 커널 모드로 전환하고 OS 코드로 점프한다.
4. 이제 OS는 프로그램이 중단된 위치를 저장하고, 다른 프로그램을 로그하고, 프로세스를 반복할 수 있다.

이를 **선점형 멀티태스킹**이라고 한다. 프로세스의 중단을 **선점**이라고 한다.
## 타임 슬라이스 계산
타임 슬라이스는 **OS 스케줄러가 프로세스를 선점하기 전에 프로세스를 실행하도록 허용하는 기간**이다. 타임 슬라이스를 선택하는 가장 간단한 방법은 모든 프로세스에 10ms 범위의 동일한 타임슬라이스를 제공하고 작업을 순서대로 순환하는 것이다. 이를 **고정 타임 슬라이스 라운드 로빈 스케줄링**이라고 한다.

고정 타임 슬라이스 스케줄링에 대한 약간의 개선 사항은 프로세스가 응답하는 데 이상적인 가장 긴 시간인 **목표 대기 시간**을 선택하는 것이다. 목표 대기 시간이란 합리적인 수의 프로세스를 가정할 때 프로세스가 선점된 후 실행을 재개하는데 걸리는 시간이다.

타임 슬라이스는 대상의 대기 시간을 총 작업 수로 나누어 계산된다. 이는 더 적은 수의 프로세스로 낭비적인 작업 전환을 제거하므로 고정된 타임슬라이스 스케줄링보다 낫다. 목표 대기 시간이 15ms이고 프로세스가 10개인 경우 각 프로세스는 15/10 또는 1.5ms 동안 실행된다. 3개의 프로세스만 있다면 5ms의 타임 슬라이스를 얻고,,,

프로세스 전환은 현재 프로그램의 전체 상태를 저장하고 다른 프로그램을 복원해야 하기 때문에 계산 비용이 많이 든다. 특정 지점을 지나면 타임 슬라이스가 너무 작으면 프로세스가 너무 빠르게 전환되어 성능 문제도 발생할 수 있다.